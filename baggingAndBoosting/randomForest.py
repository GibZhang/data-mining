...
def file2matrix(filename='sonar_all_data.txt'):
    """
    读取文件
    :param filename:
    """
...
def get_split(dataset, n_features):
    ...
    while len(features) < n_features:
        index = random.randrange(
            len(dataset[0]) - 1)  # 往 features 添加 n_features 个特征（ n_feature 等于特征数的个数），特征索引从 dataset 中随机取
        if index not in features:
            features.append(index)  # 属性扰动
